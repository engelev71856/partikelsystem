...
void ParticleSystem::setup()
{
	...
	//load image 
	emitterImage.load("images/false.png");
...
void ParticleSystem::generateAttractors(int numPaths, int numKnotsperPath)
{
	...
	for (int p = 0; p < numPaths; p++) {
		...
		for (int k = 0; k < numKnotsperPath; k++) {
			...
			//neuer Punkt zu k von Mitte	
			/*ofVec2f faktor(k, numKnotsperPath);
			ofVec2f kV = mid + faktor * sum;*/

			//ofVec2f kV = mid + ((k + 1) / numKnotsperPath) * sum;
			...
			//ofVec2f steps = mid + ((k + 1) / numKnotsperPath) * sum;
			//neuer wert ist vector von mitte zu k-ten Ring
			...
			//rotate(...) 
				//kV.rotate(ofRandom(-p*angle,p * angle), mid);
			/*if (fK > 2) {*/
				steps.rotate(angle, mid);
			/*}
			else {
				steps.rotate(ofRandom(0, angle), mid);
			}*/
		...
		//getting values
			...
			float deltaW = steps.x;
			float deltaH =  steps.y;


		//generate point
			ofVec2f knot(deltaW, deltaH);
